...
class ConverterWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Output file
        self.output_file_label = QLabel('出力ファイル（単一）:')
        self.output_file_button = QPushButton('参照')
        self.output_file_button.clicked.connect(self.browse_output_file)

        # Output folder
        self.output_folder_label = QLabel('出力フォルダ（複数）:')
        ...
        self.single_output_checkbox = QCheckBox('単一ファイルに結合する')
    ...
    def browse_input_folder(self):
        self.input_folder = QFileDialog.getExistingDirectory(self, '入力フォルダの選択')

    def browse_output_file(self):
        self.output_file, _ = QFileDialog.getSaveFileName(self, '出力ファイルの選択', '', 'JSONL ファイル (*.jsonl)')

    def browse_output_folder(self):
        self.output_folder = QFileDialog.getExistingDirectory(self, '出力フォルダの選択')
