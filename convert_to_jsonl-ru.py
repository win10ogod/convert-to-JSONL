...
class ConverterWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Папка ввода
        self.input_folder_label = QLabel('Входная папка:')
        self.input_folder_button = QPushButton('Обзор')
        self.input_folder_button.clicked.connect(self.browse_input_folder)

        # Выходной файл
        self.output_file_label = QLabel('Выходной файл (один):')
        self.output_file_button = QPushButton('Обзор')
        self.output_file_button.clicked.connect(self.browse_output_file)

        # Папка вывода
        self.output_folder_label = QLabel('Папка вывода (несколько файлов):')
        self.output_folder_button = QPushButton('Обзор')
        self.output_folder_button.clicked.connect(self.browse_output_folder)

        # Флажок для одного файла вывода
        self.single_output_checkbox = QCheckBox('Объединить в один файл')
    ...
    def browse_input_folder(self):
        self.input_folder = QFileDialog.getExistingDirectory(self, 'Выберите входную папку')

    def browse_output_file(self):
        self.output_file, _ = QFileDialog.getSaveFileName(self, 'Выберите выходной файл', '', 'Файлы JSONL (*.jsonl)')

    def browse_output_folder(self):
        self.output_folder = QFileDialog.getExistingDirectory(self, 'Выберите папку вывода')
